...
- [ ] **TOKEN-02**: The dark direction is authored with its own independently-chosen values (not derived by inverting the light direction's lightness)
- [ ] **TOKEN-03**: Type system keeps a consistent display/body/mono role structure in components — only the font-family values assigned to those roles change per direction
